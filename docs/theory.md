...
**Why this matters for specificity modeling:** the passenger domain is defined by the **A-level reaction centers**, and it does not move when the screening level changes. The A-level root carries, for each training reaction, the reaction-center atoms located by the atom mapping and the passenger as their complement (`generate_mechanistic_tree` populates `fragments` on the A-level root; each entry is `{mol, operator_indices, passenger_indices, reaction_index}`, the last naming the source reaction so a measurement can be paired with its fragment by identity rather than by order). Locating the center by the mapping rather than by substructure search is what makes it unambiguous: a bare SMARTS for the reactive pattern will also match an unrelated occurrence elsewhere in the molecule, such as the guanidinium nitrogen of arginine or the aniline nitrogen of 4-aminophenylalanine. The A-level topology is the same across all mechanistic variants, so the reaction center's atom-map numbering is carried over to every partial assigned to the same root, by remapping each partial onto the reference partial's atom map via a graph isomorphism on the A-level operator. Shared *unlabeled* topology does not by itself give a unique correspondence: when the topology has symmetries (automorphisms), several isomorphisms exist, and ESOREX fixes **one** deterministically, so a symmetric reaction center gets a consistent but arbitrary labeling rather than a canonical one. Given that fixed labeling, passenger addresses from different variants are commensurable: a bulky phenyl group adjacent to the reaction center in an ester substrate and the same group in a thioester substrate produce the same passenger [representation](representation.md), and the model can pool evidence across both.

**The abstraction level is a screening parameter, not a boundary.** Choosing D means a candidate must match the reaction center's pi-shell context to be judged feasible; it does **not** remove the pi shell from the specificity model. The atoms a C-, D-, or E-level operator matches beyond the A-level center remain passenger and keep carrying specificity weight. The two stages combine as an AND: the reaction-center environment must match a known mechanism at the requested level, **and** every atom outside the A-level reaction center contributes weight to the energetic score on top of that. Raising the screening level therefore makes the feasibility test stricter without changing what the specificity model sees.
